...
builder.Services.AddOpenTelemetry()
	.WithTracing(tracing => tracing.AddSource("AspireApp.PuiApi"))
	.WithMetrics(metrics => metrics.AddMeter("AspireApp.PuiApi"));
...
var pui = app.MapGroup("/api/pui");

pui.MapPost("/actions/{name}", async (string name, HttpContext httpContext, IHttpClientFactory httpClientFactory, Observability observability, ILoggerFactory loggerFactory) =>
{
	var logger = loggerFactory.CreateLogger("PuiApi");
	...
	if (scenarios.IsDown)
	{
		...
		return Results.Problem("PUI API is currently unavailable.", statusCode: StatusCodes.Status503ServiceUnavailable);
	}
...
pui.MapGet("/report", (Observability observability) =>
{
	return Results.Ok(new
	{
		service = "puiapi",
...
pui.MapPost("/simulations/{scenario}", (string scenario, ILoggerFactory loggerFactory) =>
{
...
pui.MapPost("/reset", (ILoggerFactory loggerFactory) =>
{
	var logger = loggerFactory.CreateLogger("PuiSimulation");
	scenarios.Reset();
	logger.LogInformation("Reset all simulation modes");
	return Results.Ok(new { scenario = "reset", enabled = false });
})
.WithName("ResetSimulation");

pui.MapGet("/simulations/state", () => Results.Ok(new
{
...
{
	private const string MeterName = "AspireApp.PuiApi";
	private const string ActivitySourceName = "AspireApp.PuiApi";
